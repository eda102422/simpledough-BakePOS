...
const Profile = () => {
  ...
  const handleVerifyCurrentPassword = async () => {
    ...
    try {
      ...
      if (success) {
        setVerified(true);
        // Do not attempt to read the actual password from the user object.
        // Leave `password` empty so the user can enter a new password if desired.
        setCurrentPassword('');
      } else {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      // Validate phone number (if provided) — digits only and exactly 11 characters
