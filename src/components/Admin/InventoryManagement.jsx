    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 sm:mb-8 gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600">Manage daily stock limits and monitor inventory levels</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={resetDailyInventory}
            className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base rounded-lg font-semibold hover:from-blue-600 hover:to-blue-700 transition-all transform hover:scale-105 shadow-lg whitespace-nowrap"
          >
            Reset Daily Stock
          </button>
        </div>
      ...
      {lowStockProducts.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg sm:rounded-xl p-4 sm:p-6 mb-6 sm:mb-8">
          <div className="flex items-center gap-3 mb-3 sm:mb-4">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-red-900">Low Stock Alerts</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {lowStockProducts.map((productId) => {
              ...
              const stock = inventory[productId];
              return (
                <div key={productId} className="bg-white rounded-lg p-3 sm:p-4 border border-red-200">
                  <h3 className="font-semibold text-sm sm:text-base text-gray-900 truncate">{product?.name}</h3>
                  <p className="text-red-600 font-bold text-sm sm:text-base">{stock?.currentStock} remaining</p>
                  <p className="text-xs sm:text-sm text-gray-600">Daily limit: {stock?.dailyLimit}</p>
                </div>
              );
            })}
          </div>
