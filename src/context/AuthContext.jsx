...
export const AuthProvider = ({ children }) => {
 ...
 // ✅ AuthContext register function
  ...
  const updateProfile = async (updates) => {
    ...
    // 4️⃣ Update local state — preserve role and metadata
    const updatedUser = data.user || {};
    const fullUser = {
      ...updatedUser,
      name: updatedUser.user_metadata?.name || updatedUser.raw_user_meta_data?.name || updates.name || user.name,
      role: updatedUser.user_metadata?.role || updatedUser.raw_user_meta_data?.role || user.role || 'customer',
      phone: updatedUser.user_metadata?.phone || updatedUser.raw_user_meta_data?.phone || updates.phone || user.phone,
      address: updatedUser.user_metadata?.address || updatedUser.raw_user_meta_data?.address || updates.address || user.address,
    };
